...
This is a potential overfitting problem. When a spectrum is noisy or has weak, ambiguous features, GaussPy+'s derivative-based detection can interpret noise fluctuations as real peaks, fitting many narrow components to chase down the residual. The result is a lower RMS &mdash; but at the cost of introducing spurious components that have no physical basis. This explains why GaussPy+ has a slightly lower _mean_ RMS despite losing the majority of head-to-head comparisons: its mean is pulled down by a tail of high-noise spectra where it fits 10&ndash;15 components to what PHSpectra correctly identifies as noise.
