...
For each spectrum we sweep $\beta$ from 2.0 to 4.5, decompose with PHSpectra, and score using Hungarian matching with the [Lindner et al. (2015)](https://arxiv.org/abs/1409.2840) criteria.
...
<figure class="scientific">
  <img src="/img/results/synthetic-f1.png" alt="Synthetic F1 vs beta" style={{maxWidth: '50%'}} />
  <figcaption>
...
1. **$F_1$ varies by only 0.040** across the full $\beta$ sweep (0.918 at $\beta=2.0$ to 0.885 at $\beta=4.5$, peaking at 0.925 at $\beta=2.8$). This confirms that $\beta$ sensitivity is low on ground-truth data.
...
<figure class="scientific">
  <img src="/img/results/synthetic-errors.png" alt="Synthetic error distributions" style={{maxWidth: '50%'}} />
  <figcaption>

**Figure 2.** Log-ratio error distributions $\ln(Q_{\rm fit} / Q_{\rm true})$ for matched component parameters at the optimal $\beta = 2.8$. From left to right: amplitude, position, and width. Box plots show the median and interquartile range for each category; the dashed line marks perfect recovery. Generated by `uv run benchmarks train-synthetic`.
