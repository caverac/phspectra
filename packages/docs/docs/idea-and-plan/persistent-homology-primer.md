...
<figure class="scientific">
  <img src="/img/results/persistence-diagram.png" alt="Persistence diagram" />
  <figcaption>

**Figure 2.** Persistence diagram for the synthetic spectrum of Figure 1. Each point represents a birth--death pair $(b, d)$; distance from the diagonal $b = d$ equals persistence. The three real peaks (red) are clearly separated from noise points clustered near the diagonal. The dashed line marks the persistence threshold $\pi_{\min} = \beta \times \sigma_{\rm rms}$ with $\beta = 3.8$ and $\sigma_{\rm rms} = 0.08$. Generated by `uv run benchmarks persistence-plot`; see [Reproducing results](../results/reproducing).

  </figcaption>
</figure>

In Figure 2, the three real peaks (red points) are clearly separated from the cluster of noise points hugging the diagonal. The dashed line parallel to the diagonal marks the persistence threshold $\pi_{\min} = \beta \times \sigma_{\rm rms}$. For this synthetic signal ($\sigma_{\rm rms} = 0.08$, $\beta = 3.8$), the threshold is $\pi_{\min} = 0.30$. Points below this line -- farther from the diagonal -- have persistence exceeding $\pi_{\min}$ and are retained as real peaks. Points above the line are rejected as noise.
