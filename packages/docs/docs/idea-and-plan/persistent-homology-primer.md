...
In this diagram, the three real peaks (red points) are clearly separated from the cluster of noise points hugging the diagonal. The dashed line parallel to the diagonal marks the persistence threshold $\pi_{\min} = \beta \times \sigma_{\rm rms}$. For this synthetic signal ($\sigma_{\rm rms} = 0.08$, $\beta = 4.0$), the threshold is $\pi_{\min} = 0.32$. Points below this line — farther from the diagonal — have persistence exceeding $\pi_{\min}$ and are retained as real peaks. Points above the line are rejected as noise.
...
flowchart TD
    ...
    C --> D["Convert peaks to<br/>initial Gaussian guesses<br/>(amplitude, mean, σ from saddle)"]
...
where $\beta = 4.0$ is the default (the main tuning parameter). The algorithm:
