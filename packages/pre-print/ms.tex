...
across $\beta \in [2.0, 4.5]$.  On 4200 real ${}^{13}$CO spectra from the
...
\begin{figure}[t!]
  ...
  \caption{%
    End-to-end pipeline used to turn spectra into physically interpretable
    components.  After noise estimation and persistence-based peak detection,
    the initial Gaussian fit is validated and scored by AICc.  If residual
    peaks or blended pairs remain, an iterative refinement loop searches for
    missed components, splits over-broad fits at negative residual dips, and
    merges redundant pairs; each modification is accepted only if it lowers
    the AICc.
    \label{fig:pipeline}}
\end{figure}
...
structure.  For each spectrum, adjacent components are ordered by centroid
velocity and the separations $\Delta v$ are measured; the resulting
distribution highlights common multi-layer spacings and can be compared
across regions with different crowding.

\begin{figure}[t!]
  \centering
  \includegraphics[width=\columnwidth]{figures/img/velocity-spacing-plot.png}
  \caption{%
    Line-of-sight layering in a single statistic.  Normalized distributions
    of velocity separations $\Delta v$ between adjacent fitted components,
    shown for all spectra (filled) and split by component count per spectrum:
    $N = 2$, $N = 3$--4, and $N \ge 5$ (step histograms).  Small $\Delta v$
...
fields ($N_{\mathrm{components}}$ and $I_{\mathrm{tot}}$) exhibit correlation
...
is used for all spectra in a survey.  While the synthetic benchmark (\S\ref{sec:synthetic}) shows that
