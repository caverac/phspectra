...
% ---------------------------------------------------------------------------
\begin{abstract}
Radio surveys deliver millions of spectra, making Gaussian decomposition
a bottleneck in turning line profiles into physical insight about the
interstellar medium.  This work presents \phspectra, a parameter-light
approach that uses
...
0.925 at the optimal $\beta = 2.8$, with performance varying by only 0.040
across $\beta \in [2.0, 4.5]$.  Compared with \gausspyp\ on 4200 real
${}^{13}$CO spectra from the Galactic Ring Survey, \phspectra\ produces lower
residual RMS in 62\% of head-to-head comparisons and runs $2.0\times$ faster,
while requiring no parameter training.  The result is a survey-ready
decomposition method that is robust to tuning choices and scalable to modern
datasets.
\end{abstract}
