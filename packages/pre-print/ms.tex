% ============================================================================
%  Persistent Homology for Parameter-Free Radio Spectral Line Decomposition
%  AASTeX v6.3.1 — AJ pre-print
% ============================================================================
\documentclass[twocolumn,twocolappendix,aj]{aastex631}
...
% --- Metadata ---------------------------------------------------------------
\shorttitle{Persistent Homology for Radio Spectral Line Decomposition}
\shortauthors{Vera-Ciro}

% ============================================================================
\begin{document}

\title{Persistent Homology for Parameter-Free Radio Spectral Line Decomposition}
...
training set does not match survey conditions; see \citet{Pun2022} for a
broader survey of persistent-homology-based machine learning that places
such approaches in context.
...
decomposition
\citep{Edelsbrunner2002,Zomorodian2005}; see \citet{Wasserman2017} for
a recent review of the broader topological-data-analysis methodology
in statistics.  Topological data analysis (TDA)
...
The most direct precedent is \citet{Shaw2025}, who use persistent homology
for source detection and deblending in 2D astronomical images.
To the best of my knowledge, this is the first application
of persistent homology to 1D spectral line decomposition.  The algorithm has a
single free parameter~$\beta$ (the persistence threshold in units of noise),
requires no expensive training, and runs in $O(n \log n)$ time.  \phspectra\
detects emission peaks only; absorption features can be recovered by
inverting the spectrum and applying the same procedure.  It is shown that it
...
\begin{figure*}[t!]
  ...
  \caption{%
    ...
    homology is applied to a synthetic nested spectrum: a broad parent with
    two narrow children embedded on its flanks, representing the
    blended-line case typical of radio spectra.  As the threshold $t$
    (dashed line) decreases, connected components are born at local maxima
    and die when they merge into an older component.  Red dots mark peak
    births; annotations show the final persistence $\pi = b - d$.
    \emph{Top left}: $t$ above all peaks (empty upper-level set).
    \emph{Top right}: the broad parent ($\pi = 2.60$) is born as the global
    maximum.  \emph{Bottom left}: both children ($\pi = 0.69$ at channel~82
    and $\pi = 0.53$ at channel~118) appear as sub-islands sitting inside
    the parent's territory.  \emph{Bottom right}: the higher-saddle child
    (channel~82) merges back into the parent and dies; the other child
    remains as a separate island.
...
\citep{Edelsbrunner2002,Edelsbrunner2008,Bonis2016}.
...
Figure~\ref{fig:waterlevel} uses the descending water-level interpretation
on a synthetic nested spectrum: a broad parent with two narrow children
embedded on its flanks, representing the blended-line case typical of radio
spectra.  As the water level drops past the parent's peak, a single broad
island is born; each child subsequently crosses the water level and appears
as a sub-island sitting topologically inside the parent's territory.  When
the water reaches the saddle separating a child from the parent, the two
islands merge and the younger component --- the child --- dies at the
saddle value.  Its persistence $\pi = b - d$ is the height drop between the
child's local maximum and that saddle, so a narrow child sitting on top of
a broad parent receives its own birth--death pair and is ranked against the
same $\pmin$ cut as any isolated peak; no explicit deblending step is
required.  The three real peaks in the figure have persistence values
$\pi = 2.60$, $0.69$, and $0.53$, well above the near-zero persistence of
noise fluctuations and above the default $\pmin = \beta\,\srms$ cut with
$\beta = 3.5$, so all three are kept as Gaussian candidates.
...
The upper-level set formulation assumes a flat, baseline-subtracted
spectrum: a sloped or broken continuum would shift raw peak heights and
saddles non-uniformly and bias the persistence ranking.  Radio spectral
lines typically sit on a flat baseline after standard calibration, so this
is automatic in the intended use case; spectra with a non-negligible
continuum component should have it subtracted prior to running \phspectra.
...
\ENSURE Model $F$ with $K$ Gaussian components
\STATE $\srms \leftarrow$ \textsc{EstimateNoise}($f$)
\STATE $\pmin \leftarrow \beta \cdot \srms$
\STATE $\{(b_j, d_j)\} \leftarrow$ \textsc{PersistenceFilter}($f$, $\pmin$)
\STATE $\{a_i, \mu_i, \sigma_i\}_{i=1}^{K} \leftarrow$ \textsc{InitialGuesses}($\{(b_j, d_j)\}$)
\STATE $F \leftarrow$ \textsc{LeastSquaresFit}($f$, $\{a_i, \mu_i, \sigma_i\}$)
\STATE $F \leftarrow$ \textsc{ValidateComponents}($F$, $\srms$)
\STATE $\aicc_{\mathrm{best}} \leftarrow$ \textsc{ComputeAICc}($f$, $F$)
\IF{no residual peaks \AND no blended pairs}
  \RETURN $F$
\ENDIF
\FOR{$i = 1$ \TO $3$}
  \STATE $r \leftarrow f - F$
  \STATE changed $\leftarrow$ \FALSE
  \FORALL{op $\in$ \{\textsc{SearchResidualPeaks}, \textsc{SplitAtNegativeDip}, \textsc{MergeBlendedPairs}\}}
    \STATE $F' \leftarrow$ op($r$, $F$, $\srms$)
    \STATE $F' \leftarrow$ \textsc{LeastSquaresFit}($f$, $F'$)
    \IF{\textsc{ComputeAICc}($f$, $F'$) $< \aicc_{\mathrm{best}}$}
      \STATE $F \leftarrow F'$;\; $\aicc_{\mathrm{best}} \leftarrow$ \textsc{ComputeAICc}($f$, $F'$)
...
\RETURN $F$
\end{algorithmic}
\caption{\phspectra\ pipeline.\label{fig:pipeline}}
\end{figure}
...
to $\beta$ across a wide range, with an optimal value at around $\approx 3$.
...
% ---------------------------------------------------------------------------
\subsection{Representative fits}\label{sec:representative}

To complement the aggregate statistics of Table~\ref{tab:categories},
Figure~\ref{fig:representative_fits} shows six individual decompositions
that span the difficulty range of the benchmark: a single bright line, a
well-separated multi-component spectrum, a heavily blended pair, a
crowded sightline with five components, an explicit failure mode, and a
representative noisy GRS sightline drawn from the test field of
\citet{Jackson2006}.  Each panel shows the input spectrum (grey step)
and the recovered \phspectra\ fit (solid black); the true component
count $N_{\mathrm{components}}$ is reported in the legend of the five
synthetic panels so that fit count and ground-truth count can be
compared at a glance, while the $F_1$ score against the full ground
truth is annotated in the lower-left corner.

\begin{figure*}[t!]
  \centering
  \includegraphics[width=\textwidth]{representative-fits.png}
  \caption{%
    Representative \phspectra\ decompositions at $\beta = 3.5$.  Each
    panel shows the data (grey step) and the summed \phspectra\ fit
    (solid black) with the recovered component count $N$ and residual
    RMS in its legend.  For the synthetic panels, the data legend also
    reports the true number of Gaussian components
    $N_{\mathrm{components}}$, and the $F_1$ score against the full
    ground-truth parameters is annotated in the lower-left corner.
    Five panels are drawn from the synthetic benchmark categories of
    Table~\ref{tab:categories} --- single bright (SB), multi separated
    (MS), multi blended (MB), and crowded (C) --- with the failure
    panel selected as the lowest-$F_1$ case in a $50$-spectrum MB+C
    scan.  The final panel is a noisy real GRS sightline (no ground
    truth) chosen to lie in the high-residual regime of
    \S\ref{sec:grs}.
    \label{fig:representative_fits}}
\end{figure*}

The four successful panels illustrate that the algorithm recovers the
correct component count and parameters across the full difficulty range,
including the MB and C categories that produce the lowest aggregate
$F_1$ values in Table~\ref{tab:categories}.  Even when components share
a saddle that drops only fractionally below the line peaks (MB), the
persistence ranking keeps each child above the $\pmin = \beta\,\srms$
threshold and the joint Levenberg--Marquardt refit recovers all
parameters within the matching tolerances of \citet{Lindner2015}.

The failure panel makes the dominant error mode visible: in highly
crowded spectra the optimiser can settle on a configuration where
components have drifted from the true positions or widths even when the
correct \emph{count} is recovered.  This shows up in the aggregate
statistics as a low $F_1$ despite a competitive residual RMS, and is
consistent with the broader difficulty trend across categories.

The noisy GRS panel demonstrates the behaviour on real data in the
regime where \gausspyp\ and \phspectra\ diverge most strongly
(\S\ref{sec:grs}).  At a recovered noise level above the
$0.2\,\text{K}$ cut, \phspectra\ keeps only those components whose
persistence and matched-filter significance exceed the threshold, so the
decomposition stays sparse where the data warrant; the recovered RMS is
dominated by the noise rather than by under-fitting.

% ---------------------------------------------------------------------------
% §3.3  GRS comparison with GaussPy+
% ---------------------------------------------------------------------------
...
\begin{figure*}[t!]
  ...
  \caption{%
    ...
    $\ell \approx 25\degr$--$35\degr$).  All maps are at the GRS native pixel
    scale of $22\arcsec$.  \emph{(a)}~Velocity RGB composite: components are
    binned by centroid velocity into three contiguous intervals
    $[-5.0, 41.7]$, $[41.7, 88.3]$, and $[88.3, 134.9]\,$km\,s$^{-1}$ mapped
    to R, G, B respectively; per-channel intensities are clipped to the 99th
    percentile.  \emph{(b)}~Topological complexity: number of Gaussian
    components per pixel, displayed on a square-root colour scale to
    separate the dense-tile core from the diffuse background.
    \emph{(c)}~Amplitude--velocity bivariate colormap: hue encodes the
    centroid velocity of the brightest component (over the recovered range
    $[-5.0, 134.9]\,$km\,s$^{-1}$) and luminance encodes its peak amplitude
    linearly, clipped to the 99th-percentile value of $3.0\,$K.
    \emph{(d)}~Dominant velocity field: centroid velocity of the brightest
    component per pixel.
    \label{fig:grs_map}}
\end{figure*}
...
filtration.  In statistical terms, persistence acts as a non-parametric
significance measure for features in noisy 1D data, an interpretation
that is well established in the applied-statistics TDA literature
\citep{Ravishanker2021}.  \gausspyp\ offers spatial coherence that
\phspectra\ currently lacks, while \rohsa\ provides full regularized
optimization with built-in coherence but requires manual tuning of
...
\url{https://github.com/caverac/phspectra} and the python package is
installable via PyPI with \texttt{pip install phspectra}.  An archived
snapshot of the version used in this work (v1.0.2) is deposited on Zenodo
\citep{PHSpectra2026}.
...
Generative AI tools were used in a limited, assistive role during the
preparation of this manuscript and its companion software, restricted to
copy-editing of the prose, code-review feedback on the implementation,
and suggestions for figure layout.  All scientific content --- the
algorithm, the benchmark design, the analysis of synthetic and real
data, the figures, and their interpretation --- was produced and
